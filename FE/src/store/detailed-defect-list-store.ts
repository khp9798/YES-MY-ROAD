...
interface DetailedDefectState {
  ...
  // 선택자(selector)
  getFilteredAndSortedData: (filterType: FilterType, processFilter: string, severityFilter: string, defectTypeFilter: string, timeRangeFilter: string, idSearchQuery: string, addrSearchQuery: string) => DetailedDamageType[]
  getCurrentPageData: (currentPage: number, itemsPerPage: number, filterType: FilterType, processFilter: string, severityFilter: string, defectTypeFilter: string, timeRangeFilter: string, idSearchQuery: string, addrSearchQuery: string) => DetailedDamageType[]
}
...
export const useDetailedDefectStore = create<DetailedDefectState>((set, get) => ({
  ...
  // 선택자
  getFilteredAndSortedData: (filterType, processFilter, severityFilter, defectTypeFilter, timeRangeFilter, idSearchQuery, addrSearchQuery) => {
    ...
    if (idSearchQuery && idSearchQuery.trim() !== '') {
      ...
      filteredData = filteredData.filter((defect) => defect.defectId.toLowerCase().includes(query))
    }
  ...
  getCurrentPageData: (currentPage, itemsPerPage, filterType, processFilter, severityFilter, defectTypeFilter, timeRangeFilter, idSearchQuery, addrSearchQuery) => {
    const filteredAndSortedData = get().getFilteredAndSortedData(filterType, processFilter, severityFilter, defectTypeFilter, timeRangeFilter, idSearchQuery, addrSearchQuery)
