...
export default function DamageSummary() {
  ...
  if (hasError) {
    return (
      <div className={`col-span-2 flex h-auto flex-col gap-4`}>
        <SummaryCard title="데이터 로드 오류" value="다시 시도해주세요" className="grow" />
      </div>
    )
  }

  // 값 처리 함수
  const formatValue = (count:number, rate:number = 0, period:string = "전날") => {
  ...
  return (
    <div className={`col-span-2 flex h-auto flex-col gap-4`}>
      ...
      <SummaryCard
        title="주간 도로파손 건수"
        value={formatValue(weeklyReport?.count, weeklyReport?.changeRate, "전주")}
        className="grow"
      />
      <SummaryCard
        title="월간 도로파손 건수"
        value={formatValue(monthlyReport?.count, monthlyReport?.changeRate, "전월")}
        className="grow"
      />
    </div>
  )
